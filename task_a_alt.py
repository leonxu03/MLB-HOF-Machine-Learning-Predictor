...
def main():
    ...
    plt.savefig("confusion_matrix_a_initial.png", dpi=500)
    plt.close()
    
    plt.figure(figsize=(20,14))
    plot_tree(model, feature_names=X.columns, class_names=y.unique().astype(str), fontsize=3)
    plt.savefig("tree_plot_a_initial.svg", format="svg") # Needs to be viewed in a web browser
    plt.close()
    ...
    disp.plot()
    plt.title("Confusion Matrix")
    plt.savefig("confusion_matrix_a_final.png", dpi=500)
    plt.close()
    
    plt.figure(figsize=(20,8))
    class_names = y.unique().astype(str)
    plot_tree(final_grid.best_estimator_, feature_names=X.columns, class_names=class_names, fontsize=7)
    plt.savefig("tree_plot_a_final.png", dpi=300, bbox_inches='tight')
